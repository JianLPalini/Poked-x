...
/*
    A variavel app recebe a instância do express. Ou seja, dentro dela, agora temos um objeto que, quando chamarmos, nos dá acesso a vários métodos
    diferentes que vamos precisar criar nosso servidor backend.
*/
...
// Estamos dizendo que nosso servidor vai utitilar e se comunicar, nas requisições e respostas, usando JSON.

/*app.use(express.json())*/

// Rotas são métodos especiais que são chamados para fazer uma determinada requisição. Cada uma tem um método HTTP (GET, POST, PUT, DELETE)! O meétodo, no express, é feito desta forma:
...
/*app.get('/mensagem', (req, res) => {
    res.status(200).send('🚀 Servidor TYPESCRIPT rodando!');
});

app.get('/meunome', (req, res) => {
    res.status(200).send("Meu nome não é Johnny!")
})
...
app.get('/usuarios', async (req, res) => {
    ...
    }catch(erro){ // se der errado, cao aqui e mostra o erro
